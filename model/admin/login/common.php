<?php
function admin_login_err_check(){
        if( empty($_POST['email']) || empty($_POST['password']) ){
                ...
                if( mysql_num_rows( $result ) == 0) {
                        ...
                        if( $password != $arr['password'] ){
                                ...
                                $_SESSION['users_id'] = $arr['id'];
                                $_SESSION['users_name'] = $GLOBALS['users_title_list'][$arr['title']] . $arr['first_name'] . "" . $arr['last_name'];
				...
				$cookie = $tmp_arr[0] . "_admin";
                                if( $_POST['autoLogin'] == "on" ) {
                                        setcookie($cookie,"$arr[id],$_POST[email],$_POST[password],$_POST[autoLogin],$_SESSION[users_name]",time() + 1825 * 24 * 60 * 60,"/");
                                }else{
