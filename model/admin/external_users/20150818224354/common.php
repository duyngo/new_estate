...
function external_users_index( $lines ){
	...
	if(strpos($_SERVER['SCRIPT_NAME'],"/external_users/")!==false){
		if(!empty( $_SESSION['external_users']['email'])){
			$sql .= " and";
			$sql .= " email like '%" . mysql_real_escape_string($_SESSION['external_users']['email']) . "%'";
		}
		if(!empty( $_SESSION['external_users']['first_name'])){
			$sql .= " and";
			$sql .= " first_name like '%" . mysql_real_escape_string($_SESSION['external_users']['first_name']) . "%'";
		}
		if(!empty( $_SESSION['external_users']['last_name'])){
			$sql .= " and";
			$sql .= " last_name like '%" . mysql_real_escape_string($_SESSION['external_users']['last_name']) . "%'";
		}
	}
	...
	if(empty($column_name)||$column_name=="first_name"){
		...
		if(!empty($_POST['first_name'])){
			if(!preg_match("/^[A-Z]+$/", $_POST['first_name'])){
				$err_msg .= "<li><a href=\"#first_name\">Please Enter [First name] in [A-Z]</a></li>";
			}
		}
		if(!empty($_POST['first_name'])){
	...
	if(empty($column_name)||$column_name=="last_name"){
		...
		if(!empty($_POST['last_name'])){
			if(!preg_match("/^[A-Z]+$/", $_POST['last_name'])){
				$err_msg .= "<li><a href=\"#last_name\">Please Enter [Last name] in [A-Z]</a></li>";
			}
		}
		if(!empty($_POST['last_name'])){
