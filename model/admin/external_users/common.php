...
function external_users_insert(){
	...
	$sql .= ",'" . mysql_real_escape_string( $_POST['published_range_of_inquiry'] ) . "'";
	$sql .= ",now()";
...
function external_users_update(){
	...
	$sql .= ",published_range_of_inquiry = '" . mysql_real_escape_string( $_POST['published_range_of_inquiry'] ) . "'";
	$sql .= ",modified = now()";
...
function external_users_err_check( $column_name ){
	...
	if(empty($column_name)||$column_name=="last_name"){
		...
		if(!empty($_POST['last_name'])){
			if(!preg_match("/^[a-zA-Z\s]+$/", $_POST['last_name'])){
				$err_msg .= "<li><a href=\"#last_name\">Please Enter [Last name] in [a-zA-Z\s]</a></li>";
			}
		}
		if(!empty($_POST['last_name'])){
	...
	if(empty($column_name)||$column_name=="published_range_of_inquiry"){
	}
	return $err_msg;
}
?>