...
function prices_listings_index( $prices_id ){
	$sql = "select";
	$sql .= " t1.*";
	$sql .= " from";
	$sql .= " listings t1";
	$sql .= " where";
	$sql .= " t1.prices_id = $prices_id";
	$sql .= " and";
	$sql .= " t1.is_deleted = 0";
	if(!empty( $_SESSION['prices']['listings']['id'])){
		$sql .= " and";
		$sql .= " t1.id like '%" . mysql_real_escape_string($_SESSION['prices']['listings']['id']) . "%'";
	}
	if(!empty( $_SESSION['prices']['listings']['name'])){
		$sql .= " and";
		$sql .= " t1.name like '%" . mysql_real_escape_string($_SESSION['prices']['listings']['name']) . "%'";
	}
	if(!empty( $_SESSION['prices']['listings']['companies_id'])){
		$sql .= " and";
		$sql .= " t1.companies_id like '%" . mysql_real_escape_string($_SESSION['prices']['listings']['companies_id']) . "%'";
	}
	if(!empty( $_SESSION['prices']['listings']['property_name'])){
		$sql .= " and";
		$sql .= " t1.property_name like '%" . mysql_real_escape_string($_SESSION['prices']['listings']['property_name']) . "%'";
	}
	if(!empty( $_SESSION['prices']['listings']['status'])){
		$sql .= " and";
		$sql .= " t1.status like '%" . $_SESSION['prices']['listings']['status'] . "%'";
	}
	...
	if(empty($column_name)||$column_name=="code"){
		...
		if(!empty($_POST['code'])){
			if(!preg_match("/^[a-z-]+$/", $_POST['code'])){
				$err_msg .= "<li><a href=\"#code\">Please Enter [Code(Url)] in [a-z-]</a></li>";
			}
		}
		if(!empty($_POST['code'])){
