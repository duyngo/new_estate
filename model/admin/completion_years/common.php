<?php
function completion_years_index( $lines ){
	$sql = "select";
	$sql .= " t1.*";
	$sql .= " from";
	$sql .= " completion_years t1";
	$sql .= " where";
	$sql .= " t1.is_deleted = 0";
	$sql .= " order by sort";
	if(!empty($lines)){
...
function completion_years_insert(){
	$sql = "insert into completion_years(";
	$sql .= " name";
	$sql .= ",code";
	$sql .= ",sort";
	$sql .= ",created";
	...
	$sql .= ")";
	common_exec_sql($sql);
	return common_get_max("completion_years");
}
function completion_years_update(){
	$sql = "update completion_years set";
...
function completion_years_delete( $id ){
	$sql = "update completion_years set";
	$sql .= " is_deleted = 1 ";
	$sql .= ",modified = now()";
	$sql .= ",modified_by = " . $_SESSION['users_id'];
...
function completion_years_err_check( $column_name ){
	$err_msg = NULL;
