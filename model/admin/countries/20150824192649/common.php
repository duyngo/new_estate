...
function countries_index( $lines ){
	...
	if(strpos($_SERVER['SCRIPT_NAME'],"/countries/")!==false){
		if(!empty( $_SESSION['countries']['code'])){
			$sql .= " and";
			$sql .= " t1.code like '%" . mysql_real_escape_string($_SESSION['countries']['code']) . "%'";
		}
		if(!empty( $_SESSION['countries']['name'])){
			$sql .= " and";
			$sql .= " t1.name like '%" . mysql_real_escape_string($_SESSION['countries']['name']) . "%'";
		}
		if(!empty( $_SESSION['countries']['logical_name'])){
			$sql .= " and";
			$sql .= " t1.logical_name like '%" . mysql_real_escape_string($_SESSION['countries']['logical_name']) . "%'";
		}
	}
	$sql .= " order by id";
	if(!empty($lines)){
...
function countries_insert(){
	...
	$sql .= ")";
	common_exec_sql($sql);
	return common_get_max("countries");
}
...
function countries_property_types_index( $countries_id ){
	...
	$sql .= " where";
	$sql .= " t1.countries_id = $countries_id";
	$sql .= " and";
	$sql .= " t1.is_deleted = 0";
	if(!empty( $_SESSION['countries']['property_types']['name'])){
		$sql .= " and";
		$sql .= " t1.name like '%" . mysql_real_escape_string($_SESSION['countries']['property_types']['name']) . "%'";
	}
	if(!empty( $_SESSION['countries']['property_types']['description'])){
		$sql .= " and";
		$sql .= " t1.description like '%" . mysql_real_escape_string($_SESSION['countries']['property_types']['description']) . "%'";
	}
	$sql .= " order by sort";
	if(!empty($lines)){
		if(empty($_REQUEST['page'])){
...
function countries_states_index( $countries_id ){
	...
	if(!empty( $_SESSION['countries']['states']['name'])){
		$sql .= " and";
		$sql .= " t1.name like '%" . mysql_real_escape_string($_SESSION['countries']['states']['name']) . "%'";
	}
	$sql .= " order by sort";
	if(!empty($lines)){
		if(empty($_REQUEST['page'])){
...
function countries_locations_index( $countries_id ){
	...
	if(!empty( $_SESSION['countries']['locations']['name'])){
		$sql .= " and";
		$sql .= " t1.name like '%" . mysql_real_escape_string($_SESSION['countries']['locations']['name']) . "%'";
	}
	$sql .= " order by name";
	if(!empty($lines)){
		if(empty($_REQUEST['page'])){
...
function countries_groups_index( $countries_id ){
	...
	$sql .= " and";
	$sql .= " t1.is_deleted = 0";
	if(!empty( $_SESSION['countries']['groups']['states_id'])){
		$sql .= " and";
		$sql .= " t1.states_id like '%" . $_SESSION['countries']['groups']['states_id'] . "%'";
	}
	if(!empty( $_SESSION['countries']['groups']['name'])){
		$sql .= " and";
		$sql .= " t1.name like '%" . mysql_real_escape_string($_SESSION['countries']['groups']['name']) . "%'";
	}
	$sql .= " order by sort";
	if(!empty($lines)){
		if(empty($_REQUEST['page'])){
...
function countries_companies_index( $countries_id ){
	...
	if(!empty( $_SESSION['countries']['companies']['name'])){
		$sql .= " and";
		$sql .= " t1.name like '%" . mysql_real_escape_string($_SESSION['countries']['companies']['name']) . "%'";
	}
	if(!empty( $_SESSION['countries']['companies']['class'])){
		$sql .= " and";
		$sql .= " t1.class like '%" . $_SESSION['countries']['companies']['class'] . "%'";
	}
	if(!empty( $_SESSION['countries']['companies']['pickup_flag'])){
		$sql .= " and";
		$sql .= " t1.pickup_flag like '%" . $_SESSION['countries']['companies']['pickup_flag'] . "%'";
	}
	$sql .= " order by name";
	if(!empty($lines)){
		if(empty($_REQUEST['page'])){
