<?php include "../../../controller/admin/states/edit.php"; ?>
<!DOCTYPE html>
<html>
<head>
<meta http-equiv="X-UA-Compatible" content="IE=edge">
<title>State Add/Edit</title>
<meta charset="utf-8">
...
if( $_POST['act'] == "err" ){
?>
	<ul class="errorTxt"><?php echo states_err_check(NULL); ?></ul>
<?php
}
?>
		<table>
			<tr id="name">
				<th>name</th>
				<td class="<?php if(states_err_check("name")){ echo "errorInput"; }?> must">
					<input name="name" size="64" id="name" class="TextInput" type="text" title="name" value="<?php echo $_POST['name'];?>">
				</td>
			</tr>
			<tr id="code">
				<th>code(URL用)</th>
				<td class="<?php if(states_err_check("code")){ echo "errorInput"; }?> must">
					<input name="code" size="32" id="code" class="TextInput" type="text" title="code" value="<?php echo $_POST['code'];?>">
				</td>
			</tr>
			<tr id="image_path">
				<th>Image</th>
				<td class="<?php if(states_err_check("image_path")){ echo "errorInput"; }?>">
<?php if(!empty($_SESSION['states']['image_path'])){ ?>
				<div>
					<div><img src="/admin/images/states/<?php echo $_SESSION['states']['image_path'];?>" width="100"></div>
					<div><input type="checkbox" name="image_path" value="delete">delete this image</div>
				</div>
<?php } ?>
				<input type="file" name="image_path" />
<p class="note">※width:223 * height:164 size required</p>				</td>
			</tr>
			<tr id="description">
				<th>Description at search result page</th>
				<td class="<?php if(states_err_check("description")){ echo "errorInput"; }?>">
					<textarea name="description" title="" rows="5" cols="50" id="description" ><?php echo $_POST['description'];?></textarea>
				</td>
			</tr>
			<tr id="sort">
				<th>sort</th>
				<td class="<?php if(states_err_check("sort")){ echo "errorInput"; }?> must">
...
?>
	<section id="editForm">
		...
		<table>
			...
			<tr>
				<th>Image</th>
				<td><?php if(!empty($_SESSION['states']['image_path'])){ ?> <img src="/admin/images/states/<?php echo $_SESSION['states']['image_path'];?>" width="100"> <?php } ?>
</td>
			</tr>
			<tr>
				<th>Description at search result page</th>
