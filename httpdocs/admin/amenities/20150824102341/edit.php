...
?>
				<option value="<?php echo $arr['id'];?>" <?php if( $arr['id'] == $_POST['states_id'] ){ echo "selected"; } ?>><?php echo $arr['name'];?></option>
<?php
}
?>
				</select>
			...
			<tr id="groups_id">
				<th>Area Group</th>
				<td class="<?php if(amenities_err_check("groups_id")){ echo "errorInput"; }?> must">
...
while( $arr = mysql_fetch_array( $result )){
?>
				<option value="<?php echo $arr['id'];?>" <?php if( $arr['id'] == $_POST['groups_id'] ){ echo "selected"; } ?>><?php echo $arr['name'];?></option>
<?php
}
?>
				</select>
...
while( $arr = mysql_fetch_array( $result )){
?>
				<option value="<?php echo $arr['id'];?>" <?php if( $arr['id'] == $_POST['amenity_categories_id'] ){ echo "selected"; } ?>><?php echo $arr['name'];?></option>
<?php
}
?>
				</select>
...
<?php if(!empty($_SESSION['amenities']['image_path'])){ ?>
				<div>
					...
					<div><input type="checkbox" name="image_path" value="delete">この画像を削除する</div>
				</div>
<?php } ?>
			...
			<tr id="map_coordinates">
				<th>Map coordinates</th>
				<td class="<?php if(amenities_err_check("map_coordinates")){ echo "errorInput"; }?> must">
					<input name="map_coordinates" size="32" id="map_coordinates" class="TextInput" type="text" title="map_coordinates" value="<?php echo $_POST['map_coordinates'];?>">
				</td>
			</tr>
		</table>
...
?>
	<section id="editForm">
		...
		<table>
			...
			<tr>
				<th>Map coordinates</th>
				<td><?php echo htmlspecialchars($_POST['map_coordinates'],ENT_QUOTES);?></td>
			</tr>
		</table>
		<table width="50%">
		<tr>
