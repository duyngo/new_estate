...
$(function(){
        $('select[name="states_id"]').change(function() {
                ...
                var count = $('select[name="locations_id"]').children().length;
                for (var i=0; i<count; i++) {
                        var locations_id = $('select[name="locations_id"] option:eq(' + i + ')');
                        if(locations_id.attr("class") === states_id) {
                                locations_id.show();
                        }else{
                                if(locations_id.attr("class") === "msg") {
                                        locations_id.show();
                                } else {
                                        locations_id.hide();
                                }
...
?>
				<option value="<?php echo $arr['id'];?>" class="<?php echo $arr['id'];?>" <?php if( $arr['id'] == $_POST['states_id'] ){ echo "selected"; } ?>><?php echo $arr['name'];?></option>
<?php
}
?>
				</select>
...
while( $arr = mysql_fetch_array( $result )){
?>
                                <option value="<?php echo $arr['id'];?>" class="<?php echo $arr['states_id'];?>" <?php if( $arr['id'] == $_POST['groups_id'] ){ echo "selected"; } ?>><?php echo $arr['name'];?></option>
<?php
}
?>
                                </select>
...
<?php } ?>
			...
			<tr id="description">
				...
				<td class="<?php if(amenities_err_check("description")){ echo "errorInput"; }?>">
					<input name="description" size="64" id="description" class="TextInput" type="text" title="description" value="<?php echo $_POST['description'];?>">
				</td>
			</tr>
			<tr id="latitude">
				<th>Latitude</th>
...
?>
	<section id="editForm">
		...
		<table>
			...
			<tr>
				<th>Description</th>
				<td><?php echo htmlspecialchars($_POST['description'],ENT_QUOTES);?></td>
			</tr>
			<tr>
				<th>Latitude</th>
