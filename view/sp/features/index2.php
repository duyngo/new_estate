...
while( $arr = mysql_fetch_array( $result )){
	...
	if( $arr['code'] != $property_type_groups_code ){       //change property_type_groups_code.
		...
		if($states_code!="all-state"||!empty($locations_code)||!empty($groups_code)||!empty($prices_code)||!empty($sizes_code)||!empty($bedrooms_code)){
			$url .= "/" . $states_code . "/";
		}
		if(!empty($locations_code)){
			$url .= "in-" . $locations_code;
...
while( $arr = mysql_fetch_array( $result )){
	...
	if( $arr['code'] != $prices_code ){     //change prices_code
		...
		if($states_code!="all-state"||!empty($locations_code)||!empty($groups_code)||!empty($property_type_groups_code)||!empty($sizes_code)||!empty($bedrooms_code)||!empty($completion_years_code)){
			$url .= "/" . $states_code . "/";
		}
		if(!empty($locations_code)){
			$url .= "in-" . $locations_code;
...
while( $arr = mysql_fetch_array( $result )){
	...
	if( $arr['code'] != $completion_years_code ){   //change completion_years_code
		...
		if($states_code!="all-state"||!empty($locations_code)||!empty($groups_code)||!empty($property_type_groups_code)||!empty($prices_code)||!empty($sizes_code)||!empty($bedrooms_code)){
			$url .= "/" . $states_code . "/";
		}
		if(!empty($locations_code)){
			$url .= "in-" . $locations_code;
...
while( $arr = mysql_fetch_array( $result )){
	...
	if( $arr['code'] != $bedrooms_code ){   //change bedrooms_code
		...
		if($states_code!="all-state"||!empty($locations_code)||!empty($groups_code)||!empty($property_type_groups_code)||!empty($prices_code)||!empty($sizes_code)||!empty($completion_years_code)){
			$url .= "/" . $states_code . "/";
		}
		if(!empty($locations_code)){
			$url .= "in-" . $locations_code;
...
?>
	...
	<div class="mdSAC01Items">
	  <div class="mdSAC01Img"><img src="/admin/images/features/<?php echo $features_image_path;?>" alt=""></div>
	  <div class="mdSAC01Txt">
	    <h2 class="Gold h2"><?php echo $features_name;?></h2>
